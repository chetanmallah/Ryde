    ...
    try {
        const timesPromises = markers.map(async (marker) => {
            ...
            const responseToDestination = await fetch(
               // `https://maps.google/maps/api/directions/json?origin=${userLatitude},${userLongitude}&destination=${destinationLatitude},${destinationLongitude}&key=${directionsAPI}`,
            ...
            const price = (totalTime * 0.1).toFixed(2); // Calculate price based on time
